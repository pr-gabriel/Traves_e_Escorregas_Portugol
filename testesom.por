programa
{
	...
	// Variáveis globais
	inteiro somApito = -1
	inteiro somApitoFinal = -1
	inteiro somTorcida = -1
	inteiro somBarbas = -1
	...
	funcao carregarAudios()
	{
		// Tenta carregar os sons. O caminho é relativo à pasta onde este .por está salvo.
		somApito = s.carregar_som("musicas/apito.mp3")
		somApitoFinal = s.carregar_som("musicas/apitofinalDoJogo.mp3")
		somTorcida = s.carregar_som("musicas/torcidaAlegra.mp3")
		somBarbas = s.carregar_som("musicas/pelasbarbasdoprofeta.mp3")
		
		// Define um volume padrão de 100 para a biblioteca
		s.definir_volume(100)
	}

	funcao tocarAudio(inteiro casa, inteiro dado)
	{
		// Toca o som do apito ao rolar o dado
		se (somApito != -1) {
			s.reproduzir_som(somApito, falso)
		}
	}
			...
			se (penalidades[pAtual] > 0) {
				...
				enquanto (turnoValido == verdadeiro) {
					...
					se (casas[pAtual] >= 25) {
						...
						se (somApitoFinal != -1) {
							s.reproduzir_som(somApitoFinal, falso)
						}
						se (somTorcida != -1) {
							s.reproduzir_som(somTorcida, falso)
						}
